...
<style>
    ...
    .stTabs [aria-selected="true"] {
        ...
        border-bottom: 3px solid #636efa !important;
    ...
    /* Expander styling */
    .node_modules-streamlit-browser-dist-index-html-560699-stExpander {
        border-radius: 12px !important;
        border: 1px solid #e2e8f0 !important;
    }
...
SCENARIOS_VN = {
    ...
    'S5': {'name': 'Tối ưu cân bằng', 'color': COLORS['Optimized'],
           'allocation': {'K': 0.35, 'D': 0.25, 'AI': 0.20, 'H': 0.20}}  # Placeholder
}
...
def tab_kpi_comparison():
    ...
    for s_key in SCENARIOS_VN.keys():
        ...
        kpi_data.append({
            ...
            'GDP 2030 (Tỷ USD)': f"{kpi['gdp_2030']:.1f}",
    ...
    with col2:
        ...
        for s_key in SCENARIOS_VN.keys():
            kpi = results[s_key]['KPI']
            values = [
                kpi['gdp_2030'] / 500 * 100,  # Normalize to 0-100
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("**Mô hình hỗ trợ ra quyết định**")
        st.markdown("Phân bổ ngân sách kinh tế số 2026-2030")
        ...
        st.markdown("""
        - **Năm dự báo**: 2026-2030
        - **Ngân sách**: 80 triệu tỷ đồng
        - **Hạng mục**: K, D, AI, H
        - **Kịch bản**: 5 chính sách
        - **Mô-đun**: 6 (M1-M6)
        """)
        
        st.divider()
        
        st.markdown("### Module thành phần")
        st.markdown("""
        - **M1**: Dự báo kinh tế (Cobb-Douglas)
        - **M2**: Đánh giá sẵn sàng (TOPSIS)
        - **M3**: Tối ưu phân bổ (LP/MIP)
        - **M4**: Mô phỏng lao động
        - **M5**: Đánh giá rủi ro
        ...
        st.markdown("### 5 kịch bản")
        for s_key, s_info in SCENARIOS_VN.items():
            st.markdown(f"- **{s_info['name']}** ({s_key})")
    
    # Header
    st.markdown("# AIDEOM-VN: Dashboard Hỗ Trợ Ra Quyết Định")
    ...
    Năm dự báo: 2026-2030 | Ngân sách tổng: 80 triệu tỷ VND | 5 kịch bản chính sách
    """)
