...
def render():
    ...
    **Mục tiêu học tập:** Sinh viên nắm vững dạng giải tích của hàm sản xuất Cobb-Douglas mở rộng, 
    ...
    with tabs[3]:
        ...
        st.markdown("Dữ liệu giả định dựa trên báo cáo GSO và mục tiêu Chiến lược kinh tế số:")
        
        data = {
            "Năm": [2020, 2021, 2022, 2023, 2024, 2025],
            "GDP (Y)": [8044, 8479, 8922, 9513, 10221, 11512],
            "Vốn (K)": [15000, 16200, 17500, 19000, 20800, 23000],
            "Lao động (L)": [53.5, 53.8, 54.1, 54.4, 54.7, 55.0],
            "Số hóa (D)": [12.5, 14.2, 16.5, 18.3, 20.5, 23.0],
            "AI (DN)": [2.0, 3.5, 5.8, 10.2, 18.5, 30.0],
            "H (ĐT)": [22.4, 23.1, 24.5, 26.2, 28.5, 31.0]
        }
        df = pd.DataFrame(data)
        st.table(df)
        st.caption("Đơn vị: GDP, Vốn (nghìn tỷ VND); Lao động, AI, H, Số hóa (chỉ số hoặc %) ")

    with tabs[4]:
        st.header("5. Kết quả & Phân tích TFP")
        
        # Coefficients
        alpha, beta, gamma, delta, theta = 0.35, 0.40, 0.10, 0.05, 0.10
        
        # Calculate TFP index (A_t)
        Y = df["GDP (Y)"].values
        K = df["Vốn (K)"].values
        L = df["Lao động (L)"].values
        D = df["Số hóa (D)"].values
        AI = df["AI (DN)"].values
        H = df["H (ĐT)"].values
        ...
        with col_res2:
            ...
            # K, L grow 6%, D -> 30%, AI -> 100k, H -> 35%, TFP grows 1.2%
            Y_2025 = Y[-1]
            K_2030 = K[-1] * (1.06**5)
            L_2030 = L[-1] * (1.06**5)
            D_2030 = 30.0
            ...
            st.success(f"Dự báo GDP Việt Nam 2030: **{Y_2030:,.2f}** nghìn tỷ VND")
