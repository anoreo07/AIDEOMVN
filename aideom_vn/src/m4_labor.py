...
def simulate_labor_market(sectors_df=None, allocation_dict=None):
    """
    Wrapper function for dashboard integration - simulates labor market impact.
    ...
    Returns:
        dict: Contains 'net_job' array and 'sectors' list
    """
    try:
        # Default sector names if no input
        if sectors_df is None:
            ...
            sectors_list = sectors_df['sector_name_vi'].tolist()[:8]
        
        # Default allocation weights if not provided
        if allocation_dict is None:
            allocation_dict = {'K': 28, 'D': 8, 'AI': 8, 'H': 8}
        
        # Simulate net job creation based on allocation
        # K: Low tech displacement, high job creation
        # D: Medium displacement, medium job creation
        # AI: High displacement, medium job creation (training offset)
        # H: Training positive impact
        
        n_sectors = len(sectors_list)
        base_displacement = np.array([120, 200, 100, 150, 80, 90, 110, 60])[:n_sectors]
        base_creation = np.array([150, 120, 140, 130, 100, 110, 200, 180])[:n_sectors]
        
        # Adjust based on allocation
        total_budget = sum(allocation_dict.values()) if isinstance(allocation_dict, dict) else 50
        h_weight = allocation_dict.get('H', 8) / total_budget if isinstance(allocation_dict, dict) else 0.16
        ai_weight = allocation_dict.get('AI', 8) / total_budget if isinstance(allocation_dict, dict) else 0.16
        
        # H (training) reduces displacement impact, AI increases it
        displacement_multiplier = 1.0 + (ai_weight * 0.5) - (h_weight * 0.3)
        creation_multiplier = 1.0 + (h_weight * 0.4)
        
        net_jobs = (base_creation * creation_multiplier - base_displacement * displacement_multiplier) + np.random.normal(0, 20, n_sectors)
        net_jobs = np.clip(net_jobs, -150, 250)
        ...
        return {
            'net_job': np.random.normal(50, 40, 8),
            'sectors': ['Sector ' + str(i) for i in range(1, 9)]
        }
